...
import { Search, Calendar, CheckCircle2, MapPin, Truck, ExternalLink, Clock } from "lucide-react";
...
export default function Agendamento() {
  ...
  const aguardando = filtered.filter(p => !p.sector_status || p.sector_status === "aguardando");
  const emAndamento = filtered.filter(p => p.sector_status === "em_producao");
    ...
    <div className="space-y-5">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground">{orders.length} ordens aguardando agendamento</p>
        </div>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          {filtered.length === 0 ? (
            <div className="col-span-3 bg-card rounded-2xl border p-8 text-center text-muted-foreground">
              ...
              return (
                <div key={po.id} className={cn(
                  ...
                  <div className="flex items-start justify-between gap-2 mb-2">
                    ...
                    <Button variant="ghost" size="sm" onClick={() => setDetailPO(po)} className="h-7 w-7 p-0 shrink-0">
                  ...
                  {po.delivery_deadline && (
                    <p className={cn("text-[11px] mb-2", isOverdue ? "text-red-500 font-semibold" : "text-muted-foreground")}>
                  ...
                  <Button
                    ...
                    onClick={() => { setScheduling(po); setScheduleDate(""); setScheduleObs(""); }}
                  >
                    <Calendar className="w-3 h-3" /> Agendar e Enviar para Expedição
                  </Button>
                </div>
              );
            })
            ...
            <div className="space-y-4">
              ...
              <div className="flex justify-end gap-2 pt-1">
                ...
                >
                  ...
                  {scheduleMutation.isPending ? "Enviando..." : "Enviar para Expedição"}
                </Button>
              </div>
            </div>
          )}
